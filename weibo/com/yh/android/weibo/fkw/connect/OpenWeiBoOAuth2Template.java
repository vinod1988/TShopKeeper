...
public class OpenWeiBoOAuth2Template implements OAuth2Operations {
	...
	protected AccessGrant postForAccessGrant(String accessTokenUrl, MultiValueMap<String, String> parameters){
		...
		try{
			...
			PostParameter[] postParameters=new PostParameter[parameters.keySet().size()];
			int i=0;
			for(String key:parameters.keySet()){
				postParameters[i]=(new PostParameter(key,parameters.getFirst(key)));
				i++;
			}
			String responseJson=weibo.doGet(accessTokenUrl, postParameters);
			JSONObject json=new JSONObject(responseJson);
			String token = json.getString("access_token");
			accessToken= new AccessGrant(token, null, null, null);
